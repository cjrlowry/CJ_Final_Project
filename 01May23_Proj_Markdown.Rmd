...
#load data, data must be saved in home workspace, remove N/A gis
urchins_raw <- read.csv("~/R/urchin_data/urchins_example_data.csv", na.strings = c("N/A", "NA"))  # location should be~/urchin_data/example_dataset/urchin_example_data.csv
...
model_biofouling <- aov(biofouling_end_rank ~ urchin_density*depth, data = urchins_cleaned) #anova
#output results
summary(model_biofouling) #summary, if we see significant results we continue:
...
Here we can view the analysis of our data using one-way ANOVAs.
...
```
This test is particularly important. Since there is no influence of cage depth on biofouling ranking we can safely assume that all the cages are functionally replicates of one another between depths (for a given urchin density); if we do see differences in biofouling ranking this would be a flag that we should perform a deeper analysis between urchin density AND depth on the response variables using an ANCOVA test (Analysis of Covariance) using the lm() function
...
```
Analysis is produced for small, medium, and large differences in means. This is useful in determining whether or not the sampling size plan we're using is appropriate to account for Type I error
